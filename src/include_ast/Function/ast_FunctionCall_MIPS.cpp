...
FunctionCall_Mips::FunctionCall_Mips(const NodePtr functionID, const std::vector<NodePtr> _Argument_expression_list)
{
    ...
    {
        //std::cout << "#<--------------------------- loop i number: " << i << " ---------------------------->" << std::endl;
...
void FunctionCall_Mips::generateMips(std::ostream &dst, Context &context, int destReg, MakeName &make_name, int &dynamic_offset)
{
    int trace_fd_num = 0 ;
    int trace_int_num = 0;
   // std::cout << "<--------------------------- function call generate ---------------------------->" << std::endl;
        ...
        for (int i = 1; i < branch.size(); i++)
        {
            if(branch[i]->get_type() == "DOUBLE" || branch[i]->get_type() == "DOUBLEPTR"){
                ...
                if (trace_fd_num < 4){
                        ...
                        dst << "move " << "$" << 4+ trace_int_num << "," << "$f0" << std::endl;
                        trace_int_num += 1;
                    }
            }
            else{
    ...
    {
        // when argument size is larger than 16
        // 
        //std::cout << "<--------------------------- inside function call else ---------------------------->" << std::endl;
        int trace_fd_num = 0 ;
        int trace_int_num = 0;
        int arg_index = 0;
        int arg_size = 0; 
    ...
    dst << "sw " << "$2," << current_offset << "($30)" << std::endl;
}
...
{
    //std::cout << "is fucnt called" << std::endl;
    return true;
}
...
{
    int arg_size = 0;
    //std::cout << "hihihi" << branch.size() << std::endl;
    for(int i = 1; i < branch.size(); i++){
        if(branch[i]->get_type() == "DOUBLE" ||  branch[i]->get_type() == "DOUBLEPTR"){
            if(arg_size % 8 != 0){
                arg_size += 12;
